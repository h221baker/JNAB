...
class DBAccountOverview(object):
    ...
    def __init__(self, account_tbl):
        ...
        if not type(account_tbl) is tinydb.database.Table:
            msg = "Expected \"account_tbl\" to be of type %s, " \
                    "but received type %s" % (repr(tinydb.database.Table),
                                                repr(type(account_tbl)))
            logger.error(msg)
    ...
    def get_all_accounts(self):
        ...
        for account in self.account_tbl.all():
            ...
            self.get_account(account_id=account['ID'],
