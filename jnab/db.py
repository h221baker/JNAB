...
class DB(object):
    ...
    def get_account(self, account_id=None, account_name=None):
        ...
        if account_id and account_name:
            ...
            accounts = self.accounts_table.search(
                    (account_query.NAME == account_name) &
                    (account_query.ID == account_id))
        elif account_id:
            logger.debug(
                    "Looking up account via account id %d" % (account_id))
            accounts = self.accounts_table.search(
                    account_query.ID == account_id)
        elif account_name:
            logger.debug(
                    "Looking up account via account name %s" % (account_name))
            accounts = self.accounts_table.search(
                    account_query.NAME == account_name)
        ...
        if not accounts:
            if account_id and account_name:
                raise DBAccountLookupError(
                        "Account ID '%d' and Account"
                        "NAME '%s' does not exist." %
                        (account_id, account_name))
            elif account_id:
                raise DBAccountLookupError(
                        "Account ID '%d' does not exist." % account_id)
            elif account_name:
                raise DBAccountLookupError(
                        "Account NAME '%s' does not exist." % account_name)
        elif len(accounts) > 1:
            raise DBAccountLookupError(
                    "Multiple accounts with similar account name found.")
        else:
    ...
    def add_account(self, account_obj):
        ...
        logger.info(
                "Adding new account %s with account id %d." %
